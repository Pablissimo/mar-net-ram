...
export class TGridBase extends VXC.TComponent {
    ...
    */
    ...
    public create() {
        ...
        this.jComponent.append($('<tfoot><tr><th><div class="grid-actionButton pull-left"/>' +
...
export class TGrid extends TGridBase {
    public onGetPage: (pageInedx: number, pageSize: number, sortDirection: string, sortColumn: TDBGridColumn, callback: (data: Array<any>) => void) => void;
...
export class TBaseGridDataSource {
    ...
    bindColumnClickEvent(_grid: TGridBase): void {
        ...
        _grid.jComponent.find("." + _grid.ID + "-row").click(function (item) {
            ...
            if (_grid.onRowClicked) _grid.onRowClicked(selectedCol);
        });
    ...
    renderdata(_data: Array<any>, _grid: TGridBase): Array<any> {
        ...
        _grid.columns.forEach((columnItem: V.TDBGridColumn) => {
            ...
            $.each(_data, function (rowIndex, item) {
                ...
                if (columnItem.onClicked != null) {
                    var cellStr = "<a style='cursor: pointer;white-space:nowrap' data-record='" + _data[rowIndex]["___RECORDID___"] + "'class='" + cellClass + "'" + tooltopflag + ">" + imageflag + StringCellVal + "</a>";
                    mappedData[rowIndex][columnItem.ID] = cellStr;
                } else {
                    mappedData[rowIndex][columnItem.ID] = "<div data-record='" + _data[rowIndex]["___RECORDID___"] + "'class='" + cellClass + "'" + tooltopflag + " style='white-space:nowrap'>" + imageflag + StringCellVal + "</div>";;
                }
            })
            return true;
        });
